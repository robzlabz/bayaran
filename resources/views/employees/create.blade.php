<x-app-layout>
    ...
    <div class="py-6 px-4 sm:px-6 lg:px-8 max-w-2xl mx-auto">
        ...
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
            <form method="POST" action="{{ route('company.employees.store') }}" class="p-6 space-y-6">
                @csrf
                ...
                {{-- No HP --}}
                <div>
                    <label for="phone" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">No. HP <span class="text-gray-400 font-normal">(untuk login)</span></label>
                    <input type="text" id="phone" name="phone" value="{{ old('phone') }}" required
                           class="block w-full rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm px-4 py-2.5"
                           placeholder="0812xxxxxxxx">
                    <p class="mt-1 text-xs text-gray-400">Password default: <strong>bayaran</strong> + 4 digit terakhir no HP</p>
                </div>
                    ...
                    <div class="grid grid-cols-4 gap-3">
                        ...
                        <label class="relative flex cursor-pointer rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 p-4 shadow-sm focus:outline-none has-[:checked]:border-green-500 has-[:checked]:ring-1 has-[:checked]:ring-green-500">
                            <input type="radio" name="payment_type" value="daily" class="sr-only"
                                   {{ old('payment_type') === 'daily' ? 'checked' : '' }}
                                   onchange="togglePaymentFields()">
                            <div class="text-center w-full">
                        ...
                        <label class="relative flex cursor-pointer rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 p-4 shadow-sm focus:outline-none has-[:checked]:border-yellow-500 has-[:checked]:ring-1 has-[:checked]:ring-yellow-500">
                            <input type="radio" name="payment_type" value="per_delivery" class="sr-only"
                                   {{ old('payment_type') === 'per_delivery' ? 'checked' : '' }}
                                   onchange="togglePaymentFields()">
                            <div class="text-center w-full">
                ...
                {{-- Nominal Fields --}}
                <div id="field-monthly" class="payment-field {{ old('payment_type') === 'monthly' ? '' : 'hidden' }}">
                    ...
                    <div class="relative">
                        ...
                        <input type="number" id="salary_amount" name="salary_amount" value="{{ old('salary_amount') }}"
                ...
                <div id="field-daily" class="payment-field {{ old('payment_type') === 'daily' ? '' : 'hidden' }}">
                ...
                <div id="field-delivery" class="payment-field {{ old('payment_type') === 'per_delivery' ? '' : 'hidden' }}">
                    ...
                    <div class="relative">
                        <span class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-sm">Rp</span>
                        <input type="number" id="delivery_rate" name="delivery_rate" value="{{ old('delivery_rate') }}"
